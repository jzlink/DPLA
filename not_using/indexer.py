...
def test():
    dotstring = 'sourceResource.collection.date'
    value = find(dotstring)
    if value !=False:
        print "%s is %s" %(dotstring, value)
    else:
        print'%s is not in the document' %dotstring
